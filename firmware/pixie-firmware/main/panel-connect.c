...
static bool getAccountAddress(FfxAddress *addressOut) {
    FfxEcPrivkey privkey;
    if (!ffx_deviceTestPrivkey(&privkey, ACCOUNT_INDEX)) { return false; }

    FfxEcPubkey pubkey;
    bool ok = ffx_ec_computePubkey(&pubkey, &privkey);
    memset(privkey.data, 0, sizeof(privkey.data));
    if (!ok) { return false; }

    *addressOut = ffx_eth_getAddress(&pubkey);
    return true;
}

static void appendShortAddress(void *info, const char *title,
  const FfxAddress *address) {
    FfxChecksumAddress checksum = ffx_eth_checksumAddress(address);

    char str[14];
    memcpy(&str[0], &checksum.text[0], 6);
    str[6] = '.';
    str[7] = '.';
    str[8] = '.';
    memcpy(&str[9], &checksum.text[38], 5);

    ffx_appendInfoEntry(info, title, str, NULL, NULLARG);
}

static FfxDataResult stripLeadingZeros(FfxDataResult value) {
    while (value.length && value.bytes[0] == 0) {
        value.bytes++;
        value.length--;
    }
    return value;
...
static FfxDataResult getDataParam(FfxCborCursor *params, const char *key) {
    FfxCborCursor cursor = ffx_cbor_followKey(params, key);
    if (cursor.error) {
        return (FfxDataResult){ .error = cursor.error };
    }
    if (!ffx_cbor_checkType(&cursor, FfxCborTypeData)) {
        return (FfxDataResult){ .error = FfxDataErrorBadData };
    }
    return ffx_cbor_getData(&cursor);
}

static FfxDataError parseAuthRequest(AuthRequest *auth,
  FfxCborCursor *params) {
    auth->chainId = stripLeadingZeros(getDataParam(params, "chainId"));
    if (auth->chainId.error) { return auth->chainId.error; }
    if (auth->chainId.length > 32) { return FfxDataErrorOverflow; }

    auth->contractAddress = getDataParam(params, "contractAddress");
    if (auth->contractAddress.error) { return auth->contractAddress.error; }
    if (auth->contractAddress.length != 20) { return FfxDataErrorBadData; }

    auth->nonce = stripLeadingZeros(getDataParam(params, "nonce"));
    if (auth->nonce.error) { return auth->nonce.error; }
    if (auth->nonce.length > 8) { return FfxDataErrorOverflow; }
...
static int initAuthFunc(void *info, void *_state, void *arg) {
    AuthRequest *auth = arg;

    FfxAddress account;
    if (getAccountAddress(&account)) {
        appendShortAddress(info, "ACCOUNT", &account);
    }

    appendNetwork(info, &auth->chainId);
    appendAddress(info, "DELEGATE", &auth->contractAddress, &auth->chainId);
    appendDecimal(info, "NONCE", 0, &auth->nonce);

    ffx_appendInfoButton(info, "REJECT", COLOR_CANCEL, clickReject, NULLARG);
    ffx_appendInfoButton(info, "SIGN AUTH", COLOR_APPROVE, clickApprove,
      NULLARG);

    return 0;
}

static bool pushPanelAuthorization(AuthRequest *auth) {
    return ffx_pushInfo(initAuthFunc, "EIP-7702 AUTH", 0, auth);
}

static FfxDataError hashAuthorization(FfxEcDigest *digestOut,
  const AuthRequest *auth) {
    uint8_t payload[128] = { 0 };
    payload[0] = EIP7702_MAGIC;

    FfxRlpBuilder rlp = ffx_rlp_build(&payload[1], sizeof(payload) - 1);
    ffx_rlp_appendArray(&rlp, 3);
    ffx_rlp_appendData(&rlp, auth->chainId.bytes, auth->chainId.length);
    ffx_rlp_appendData(&rlp, auth->contractAddress.bytes,
      auth->contractAddress.length);
    ffx_rlp_appendData(&rlp, auth->nonce.bytes, auth->nonce.length);

    size_t rlpLength = ffx_rlp_finalize(&rlp);
    if (rlp.error) { return rlp.error; }

    ffx_hash_keccak256(digestOut->data, payload, 1 + rlpLength);
    return FfxDataErrorNone;
}

static void replySignAuthorization(uint32_t messageId,
  const AuthRequest *auth) {
    FfxEcDigest digest;
    FfxDataError error = hashAuthorization(&digest, auth);
    if (error) {
        ffx_sendErrorReply(messageId, error, "failed to hash authorization");
        return;
    }
    ...
    printf("auth sig: status=%ld\n", status);

    memset(privkey.data, 0, sizeof(privkey.data));

    if (!status) {
        ffx_sendErrorReply(messageId, 2, "failed to sign authorization");
        return;
    }

    uint8_t yParity = sig.data[64];
    if (yParity >= 27) { yParity -= 27; }

    uint8_t replyBuffer[192];
    FfxCborBuilder reply = ffx_cbor_build(replyBuffer, sizeof(replyBuffer));

    ffx_cbor_appendMap(&reply, 6);
    ffx_cbor_appendString(&reply, "chainId");
    ffx_cbor_appendData(&reply, auth->chainId.bytes, auth->chainId.length);
    ffx_cbor_appendString(&reply, "contractAddress");
    ffx_cbor_appendData(&reply, auth->contractAddress.bytes,
      auth->contractAddress.length);
    ffx_cbor_appendString(&reply, "nonce");
    ffx_cbor_appendData(&reply, auth->nonce.bytes, auth->nonce.length);
    ffx_cbor_appendString(&reply, "yParity");
    ffx_cbor_appendNumber(&reply, yParity);
...
*/
    ...
    } else if (strcmp(method, "ffx_signAuthorization") == 0) {
        AuthRequest auth = { 0 };
        FfxDataError error = parseAuthRequest(&auth, &params);
        if (error) {
            ffx_sendErrorReply(messageId, error, "invalid authorization");
            return;
        }

        bool accept = pushPanelAuthorization(&auth);
        printf("AUTH: %d\n", accept);

        if (accept) {
            replySignAuthorization(messageId, &auth);
        } else {
            ffx_sendErrorReply(messageId, 1000, "user rejected request");
        }
