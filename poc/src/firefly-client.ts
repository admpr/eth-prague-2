...
function fireflyProtocolError(status: number): Error {
  switch (status) {
    case ERROR_UNSUPPORTED_VERSION:
      return new Error("Unsupported Firefly protocol version");
    case ERROR_BAD_COMMAND:
      return new Error("Firefly rejected an unknown command");
    case ERROR_BUFFER_OVERRUN:
      return new Error("Firefly message buffer overrun");
    case ERROR_MISSING_MESSAGE:
      return new Error("Firefly is missing a message chunk");
    case ERROR_BAD_CHECKSUM:
      return new Error("Firefly reported a bad message checksum");
    case ERROR_BUSY:
      return new Error("Firefly is busy");
    case ERROR_UNKNOWN:
      return new Error("Firefly reported an unknown error");
    default:
      return new Error(`Firefly protocol error 0x${status.toString(16)}`);
  }
}
...
export class FireflyClient {
  ...
  get serialNumber(): number {
    if (!this.queryCache) throw new Error("Firefly query not loaded");
  ...
  get model(): string {
    ...
    return `Unknown Firefly model 0x${model.toString(16)}`;
  }
  ...
  async destroy(): Promise<void> {
    ...
    for (const pending of this.pendingReplies.values()) {
      ...
      pending.reject(new Error("Firefly disconnected"));
    }
  ...
  static async discover(forceRequest = false): Promise<FireflyClient> {
    ...
    if (query.version !== 0x01) {
      throw new Error(`Unsupported Firefly protocol version ${query.version}`);
    }
    if (query.length) {
      await firefly.reset();
    }
  ...
  private handleContent(data: Uint8Array): void {
    ...
    if (status === CMD_CONTINUE_MESSAGE) {
      ...
      if (expectedOffset !== this.incoming.offset) {
        this.rejectAll(new Error("Firefly message chunks arrived out of order"));
        return;
      }
  ...
  private async processIncomingIfComplete(): Promise<void> {
    ...
    if (!checksum.every((byte, index) => byte === expected[index])) {
      this.rejectAll(new Error("Firefly response checksum mismatch"));
      this.incoming.offset = 0;
      this.incoming.length = 0;
      return;
    }
  ...
  private async readResult(): Promise<Uint8Array> {
    ...
    throw new Error("Timed out waiting for Firefly response");
  }
